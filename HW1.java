 ...
 */
 ...
 // Case when the head node has no value,remove it
             ...
             public void removeElement ( int value ) {
                ...
                while (this.head != null && this.head.data == value) {
                 this.head = head.next; // Move head to the next node
             }
 
             // Now check the rest of the list and remove nodes with the specified value
             Node current = this.head;
             while (current != null) {
    ...
    // Copy original stack to tempStack and originalStack (to restore it later)
    while (!stack.isEmpty()) {
        int value = stack.pop();
        tempStack.push(value);
        originalStack.push(value);  // This preserves the original stack for later
    }

    // Now restore the original stack from the originalStack
    while (!originalStack.isEmpty()) {
        stack.push(originalStack.pop());
    }

    // Start looking for the largest index of `k`
    int largestIndex = -1;
    int currentIndex = 0;

    // Iterate over tempStack to find the largest index for value `k`
    ...
    // Return the largest index if found, or 0 if not found
    return largestIndex == -1 ? 0 : largestIndex;
}
     }
